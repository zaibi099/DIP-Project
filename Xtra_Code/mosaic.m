...
change_y = left_top(1,2) - right_top(1,2);
change_x = left_top(1,1) - right_top(1,1);

slope = change_y / change_x

%y = slope*(x − right_top(1,1)) + right_top(1,2);
linee = zeros(w1,2);
EqVal = round(slope*(right_top(1,1)) + right_top(1,2));
...
%im(left_top(1):right_top(1),left_top(2):right_top(2)) = 0;
...
%I3 = imcrop(outputImage,[co ro coMax roMax]);
figure, imshow(out_x,[]) , title('Output Image');
imwrite(out_x,gray(256),namee);
%figure, imshow(I3,[]),title('Output Image');

%Biggest_Region(I3);
%imwrite(I3,gray(256),'outx.jpg');
